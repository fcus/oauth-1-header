...
import { OAuthToHeaderOptions } from './oauth.type';
...
test('encodeParameters', t => {
  ...
  t.is(
    result,
    'a%3D1%26b%3D2%26b%3D3%26c%3D4%26oauth_body_hash%3DbodyHash%26oauth_consumer_key%3Dkey%26oauth_nonce%3Dnonce%26oauth_signature%3Dsignature%26oauth_signature_method%3DHMAC-SHA256%26oauth_timestamp%3D1%26oauth_token%3Dtoken%26oauth_version%3D1.0',
  );
});
...
test('toHeader', t => {
  ...
  t.deepEqual(header, OAuth.generateHeader(result));
  t.is(
    result.toHeader(),
...
test('toHeader with options', t => {
  ...
  t.deepEqual(header, OAuth.generateHeader(result, toHeaderOpts));
  t.is(
    result.toHeader(),
    `OAuth realm="realm"; oauth_consumer_key="key"; oauth_nonce="${
      result.oauth_nonce
    }"; oauth_signature="${OAuth.encode(
      result.oauth_signature,
    )}"; oauth_signature_method="HMAC-SHA1"; oauth_timestamp="${
